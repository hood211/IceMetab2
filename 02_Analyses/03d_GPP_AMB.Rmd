...
# Load & munge data
```{r}
load(file.path(here::here("04_SavedImages"), "03d_GPPanalyses_AMB_rdat"))
```
...
Removed DOYres because of concurvity
```{r echo=FALSE, include = FALSE}
# g.modsel_t <- uGamm(lGPP ~ invKT.C.StMean  + s(LightPerDay.lcg),
#                 correlation = BestGPPCor, REML = FALSE, data = met,
#                 control = lmeControl(maxIter = 1e8, msMaxIter = 1e8, niterEM = 1000), class = "gamm")
# 
# g.modsel_s <- uGamm(lGPP ~ stream  + s(LightPerDay.lcg),
#                 correlation = BestGPPCor, REML = FALSE, data = met,
#                 control = lmeControl(maxIter = 1e8, msMaxIter = 1e8, niterEM = 1000), class = "gamm")
# 
# 
...
write_csv(g.DredgeSumF,
          file.path(here::here("06_AICtables"), "03d_GPP_AMB.csv"))
```


```{r}
g.gpp.BEST <- gamm(lGPP ~  invKT.C.StMean + s(LightPerDay.lcg),
                correlation = BestGPPCor, REML = TRUE, data = met,
                control = lmeControl(maxIter = 1e8, msMaxIter = 1e8, niterEM = 1000))
```

```{r}
g.gpp.BEST_s <- gamm(lGPP ~  stream + s(LightPerDay.lcg),
                correlation = BestGPPCor, REML = TRUE, data = met,
                control = lmeControl(maxIter = 1e8, msMaxIter = 1e8, niterEM = 1000))
```
...
```{r echo=FALSE}
ggplot(metBestTest, aes(y = gppres1, x = DOYres, color = stream, shape = treatment)) +
  geom_point() +
...
```{r echo = FALSE, include = FALSE}
 # saveRDS(g.gpp.BEST,
 #         file = file.path(here::here("05_SavedModels"), "03d_GPP_AMB_Best.rds"))

```


```{r}
# save.image(file.path(here::here("04_SavedImages"), "03d_GPPanalyses_AMB_rdat"))
```
