...
write_csv(g.DredgeSumF,
          file.path(here::here("06_AICtables"), "03a_NEP_P.csv"))
```



```{r}
g.nep.BEST <- gamm(NEP ~ treatment * stream  + s(LightPerDay.lcg),
...
```{r}
g.nep.BEST_t <- gamm(NEP ~ treatment * invKT.C.StMean    + s(LightPerDay.lcg),
                correlation = BestNEPCor, REML = TRUE, data = met,
                control = lmeControl(maxIter = 1e8, msMaxIter = 1e8, niterEM = 1000))
```
...
```{r}
round(concurvity(g.nep.BEST_t$gam, full = FALSE)$estimate,1)
```
